...
impl NewsAPI {
    pub fn new(api_key: &str) -> NewsAPI {
        NewsAPI {
            ...
            endpoint: Endpoit::TopHeadLines,
            country: Country::Us,
        }
    }

    pub fn endpoint(&mut self, endpoint: Endpoit) -> &mut NewsAPI {
...
fn map_response_err(code: Option<String>) -> NewsApiError {
    if let Some(code) = code {
        match code.as_str() {
            ...
            _ => NewsApiError::BadRequest("Unknow error"),
        }
    } else {
        NewsApiError::BadRequest("Unknow error")
    }
}
